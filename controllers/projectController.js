...
exports.create = async(req,res)=>{
    const schema = Joi.object({     
       ...
       try{
        ...
        await  Project.create(payload, (err,data)=>{
             if(err)throw err
...
exports.list = async(req,res)=>{
    var pageNo = parseInt(req.body.startNumber)
     ...
     query.limit = size;
        try {
            await Project.find({},{},query,(err, data)=>{
                if(err)throw err
                data.sort((a,b)=>{
                ...
                Project.countDocuments({},(count_error, count) => {
                    if (err) {
...
exports.listbyid = async(req,res)=>{
    ...
    // query.productid = req.params.productid;
        try {
            await Project.find(reqparams, {},query,(err, data)=>{
                if(err)throw err
                data.sort((a,b)=>{
                    return new Date(b.creation_dt) - new Date(a.creation_dt);
                });
    ...
    //     }
      ...
      let arr = [];
        ...
        for (let x in payload) {
         ...
         try {
            await Project.find({"$or":[filterCond]}, {},query,(err, data)=>{
                if(err)throw err
                data.sort((a,b)=>{
                    return new Date(b.creation_dt) - new Date(a.creation_dt);
                });
                ...
                Project.countDocuments({"$or":[filterCond]},(count_error, count) => {
                    if (err) {
                      return res.json(count_error);
                    }
                    return res.json({
...
exports.update = async (req,res)=>{
    ...
    if(req.files.length>0){
        ...
        const pInfo = await Project.findById(id); 
        if(pInfo){
    ...
    try{ 
             const data = await Project.findByIdAndUpdate( req.params.id,payload,{new:true})
...
exports.delete = async(req,res)=>{
    try{
        await Project.findByIdAndDelete(req.params.id,(err,data)=>{
            if(err)throw err
